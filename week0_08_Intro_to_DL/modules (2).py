...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        local_derivative = np.stack([
            -np.outer(local_softmax[i, :], local_softmax[i, :]) + np.diag(local_softmax[i, :].flatten())
                                     for i in range(local_softmax.shape[0])
        ])
        self.gradInput = np.matmul(gradOutput[:, np.newaxis, :], local_derivative)[:, 0, :]        
        return self.gradInput
...
class LogSoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        local_derivative = local_derivative - local_softmax[:, np.newaxis, :]
...
class BatchNormalization(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput = (m * gradOutput - np.sum(gradOutput, axis=0) - self.output * np.sum(gradOutput * self.output, axis=0))                             / (m * np.sqrt(np.var(input, axis=0) + self.EPS))
...
class Dropout(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training == True:
            self.mask = (np.random.rand(*input.shape) > self.p) * 1/(1-self.p)
...
class SoftPlus(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        local_derivative = 1 / (1 + np.exp(-input))
        self.gradInput = np.multiply(gradOutput, local_derivative)
        return self.gradInput
...
class ClassNLLCriterionUnstable(Criterion):
    ...
    def updateOutput(self, input, target): 
        ...
        # Your code goes here. ################################################
        self.output = -np.mean(np.log(input_clamp[target == 1]))
    ...
    def updateGradInput(self, input, target):
        ...
        self.gradInput[np.where(target==1)] = (1/input.shape[0] * (-1/input_clamp[np.where(target == 1)]))
...
class ClassNLLCriterion(Criterion):
    ...
    def updateOutput(self, input, target): 
        # Your code goes here. ################################################
        self.output = -np.mean(input[target==1])
        return self.output

    def updateGradInput(self, input, target):
        # Your code goes here. ################################################
        self.gradInput = np.zeros(input.shape)
        self.gradInput[np.where(target==1)] = -1/input.shape[0]
        return self.gradInput
...
scipy.signal.correlate(W, Z, mode="valid")
...
class Conv2d(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = np.zeros((batch_size, self.out_channels,
        ...
        for i in range(input.shape[0]):
            for j in range(self.W.shape[0]):
                self.output[i][j] = scipy.signal.correlate(input[i], self.W[j], mode="valid") + self.b[j]
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for i in range(input.shape[0]):
            for j in range(input.shape[1]):   
                self.gradInput[i][j] = scipy.signal.correlate(gradOutput[i], self.W[:,j,::-1,::-1], mode="valid")
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        for i in range(self.W.shape[1]):
            for j in range(self.W.shape[0]):   
                self.gradW[j][i] = np.sum(scipy.signal.correlate(input[:,i], gradOutput[:,j], mode="valid"), axis=0)     
...
class MaxPool2d(Module):
    ...
    def updateOutput(self, input):
        ...
        # YOUR CODE #############################
        self.output = np.reshape(input, (input.shape[0], input.shape[1],
                                         input.shape[2] // self.kernel_size,
                                         self.kernel_size, 
                                         input.shape[3] // self.kernel_size,
                                         self.kernel_size))
        self.output = np.swapaxes(self.output, 3, 4)
        self.output_ = np.amax(self.output, axis=(4,5), keepdims=True)

        self.local_derivative = self.output_ == self.output
        return np.squeeze(self.output_, axis=(4,5))
    
    def updateGradInput(self, input, gradOutput):
        # YOUR CODE #############################
        local_derivative = self.local_derivative
        self.gradInput = (local_derivative * gradOutput[:, :, :, :, None, None])
